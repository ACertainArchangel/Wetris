...
Misc. we had on hand & \$0.00 & Jumper wires, buttons, tools, resistors, breadboard, 9V battery, USB cable, opto-couplers etc. \\ \hline
...
Standard 10k ohm rotary potentiometer used to adjust game speed.
...
4N35 optocouplers with a forward voltage of \qty{1.18}{\volt} and a maximum collector-emitter voltage of \qty{30}{\volt} used to isolate the high current 12V actuator control circuit from the low voltage 5V logic of the Arduino.
More information can be found at: \\
...
For this project we designed a series of peripherals to interface with the microcontroller in the Arduino Mega 2560 development board, and connected them in a 3D printed casing. The design is highly modular, with each peripheral being controlled by a library, and being handled from a high-level main loop. See code and schematics for most in-depth explanation. The peripherals are:
\begin{itemize}
...
\item The potentiometer module, which is used to adjust the speed of the Tetris module. (Fuse protected.)
\end{itemize}


\subsection{Hardware Architecture}

\subsubsection{Microcontroller Resources Table}
The following table outlines the microcontroller resources allocated to each component of the Wetris system, and details some of the connections used. Full schematic diagrams give all connection details and are included later in this document.
...
OC1A(pin11)+ Timer1 & Gabe & Square wave output for music \\ \hline
OC3A(pin5) + Timer3 & Gabe & Square wave output for music \\ \hline
OC4A(pin6) + Timer4 & Gabe & Square wave output for music \\ \hline
PCINT7 (pin13) & Roman+Jorge+Gabe & Left Movement \\ \hline
PCINT6 (pin12) & Roman+Jorge+Gabe & Right Movement \\ \hline
PCINT4 (pin10) & Roman+Jorge+Gabe & Rotation Movement \\ \hline
Pins for LCD (pins 7-9) & Roman & LCD connections \\ \hline
Pins for LCD(Dig. Pins50-52) & Roman & LCD connections \\ \hline
Pin for LCD LED (pin2) & Roman & Backlight control \\ \hline
FWD Pin (22) & Son+Gabe+Jorge & Drive actuator forward \\ \hline
REV Pin (23) & Son+Gabe+Jorge & Drive actuator reverse \\ \hline
A0 (pin A0) & Son & Potentiometer for game speed adjustment \\ \hline
\end{tabular}
\caption{Hardware resource allocation for Wetris system}
\label{tab:hardware}
\end{table}

\subsubsection{LCD Tetris Module}
In order to reach the back of the casing where the MCU is mounted from the LCD and buttons in the front of the casing, we used 16 AWG wire to connect the buttons, and we used Dupont wires linked end to end to connect the LCD. The LCD and button connections in detail are as follows:
\begin{itemize}
    \item Pin 2  $\rightarrow$ LED pin on the LCD
    \item Pin 7 $\rightarrow$ CS pin on LCD
    \item Pin 8  $\rightarrow$ RESET pin on LCD
    \item Pin 9  $\rightarrow$ CD pin on LCD
    \item Pin 10  $\rightarrow$ Rotate Piece Button
...
The music module uses output compare pins OC1A (pin 11), OC3A (pin 5), and OC4A (pin 6) in CTC (Clear Timer on Compare) mode to generate square wave signals at specific frequencies for three separate piezo buzzers. Each buzzer is connected in series with a 100 ohm resistor to limit current and a \qty{10}{\micro\farad} capacitor to smooth the signal. The code directly loads frequency and duration values into memory and iterates through them to play the song. This method is highly memory intensive, but allows for accurate timing and frequency control. The music was converted from midi format to c style arrays using a custom python script found in the repository.

\subsubsection{Water Gun Module}
In order to isolate the high current 12V power supply used to drive the water gun actuator from the 5V logic of the Arduino, we used a relay module controlled by opto-couplers. The relay module is powered by the 12V supply, and the opto-couplers are powered by the 5V supply from the Arduino. The opto-couplers are connected to digital pins 22 and 23 on the Arduino, which control the forward and reverse movement of the actuator respectively. When the game is over and the safety threshold has not been reached, the Arduino activates the relay in reverse to trigger the water gun mechanism. The forward function is not used in normal operation, but is included for completeness. Very importantly, an inline fuse is included in the 12V power line to protect against short circuits.

\subsubsection{Potentiometer Module}
The potentiometer is our most simple peripheral, being connected to the 5V and GND pins on the Arduino, with the wiper connected to analog pin A0. The potentiometer is used to adjust the speed of the game, allowing the player to customize their experience. Also allowing their friends to set it to an impossible speed in the middle of a game for extra fun.
...
\caption{Game over screen with state information shown on serial monitor}
\label{fig:wash}
\end{figure}
...
\subsection{High Level System Schematic}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{photos/High_Level_Schematic.pdf}
\caption{High level electrical schematic of Wetris system}
\label{fig:schematic-high-level}
\end{figure}

\subsection{Water Gun Module Schematic}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{photos/Water_Gun_Schematic.pdf}
\caption{Electrical schematic of water gun module}
\label{fig:schematic-water-gun}
\end{figure}

\subsection{Music Module Schematic}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{photos/Music_Schematic.pdf}
\caption{Electrical schematic of music module}
\label{fig:schematic-music}
\end{figure}

\subsection{LCD and Button Module Schematic}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{photos/LCDTetris_Schematic.pdf}
\caption{Electrical schematic of LCD and button module}
\label{fig:schematic-lcd-buttons}
\end{figure}

\subsection{Potentiometer Module Schematic}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{photos/Potenteometer_Schematic.pdf}
\caption{Electrical schematic of potentiometer module}
\label{fig:schematic-potentiometer}
\end{figure}

\subsection{3D Casing Technical Drawings}
...
\begin{minipage}[t]{0.65\textwidth}
    ...
    I am an electrical engineering student in the dual enrollment program at UNR, and as of fall 2025 I am 17 years old. I plan to graduate with my bachelor's degree in 2027 and move on to a master's and PhD in robotics. Embedded systems and machine learning are my main interests in terms of engineering. I enjoy hiking, cooking, studying the Bible, playing the cello, and working with my hands. I speak Afrikaans, English, and really awful Spanish. I have also started trying to learn Russian and am minoring in entrepreneurship.
\end{minipage}
...
\begin{minipage}[t]{0.3\textwidth}
    \vspace{0pt}
    \centering
    \includegraphics[width=3cm]{photos/Dieufainson_Jean.jpeg}
...
\begin{minipage}[t]{0.3\textwidth}
    \vspace{0pt}
    \centering
    \includegraphics[width=3cm]{photos/Jorge_Robles.jpg}
...
\begin{minipage}[t]{0.65\textwidth}
    ...
    Used GPIO methods to write code that did not implement any functions such as digitalWrite, digitalRead etc. Also added the interrupts, ISR, for the hardware buttons instead of polling. New water gun actuator control software.
...
\begin{minipage}[t]{0.3\textwidth}
    \vspace{0pt}
    \centering
    \includegraphics[width=3cm]{photos/Roman_Rosburg.jpg}
...
\begin{minipage}[t]{0.65\textwidth}
    ...
    \textbf{GitHub}: \url{https://github.com/RomanRosburg}
